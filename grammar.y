...
construct_repeat: 
    T_REPEAT
    {
      // First semantic action: Store the entry number of the next instruction to be generated (PART 1)
      // TODO: store in the stack the entry number of the next instruction to be generated (use @$.begin.line instead of $$)
      @$.begin.line = INSTRUCTION_NEXT; // CHANGED HERE
    }
    stmt_list 
    T_UNTIL 
    T_LPAR 
    ...
    T_RPAR
    {
      // Second semantic action: Jump-if-zero (OP_JZ) back to the start if the condition evaluates to false
      // TODO: Retrieve the value stored in the stack in the first semantic action
      // above (the second symbol)
      int jump_dst = @2.begin.line; // Retrieve the address stored in the first semantic action -- CHANGED HERE
      // TODO: Generate a jump-if-zero (OP_JZ) to the address stored in the first semantic
      // action of this rule
      itab_instruction_add (itab, OP_JZ, $6->addr, NOARG, jump_dst); // -- CHANGED HERE
    }
    ;
